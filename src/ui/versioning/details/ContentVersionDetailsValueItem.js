...
const renderImageAttributeValue = (attribute,
  defLangCode, currentLangCode, domain, showLabel = true) => {
  const { code, values } = attribute;
  const target = values[currentLangCode] || values[defLangCode];
...
const renderPlainAttributeValue = (attribute,
  defaultLangCode, currentLangCode, showLabel = true) => {
...
renderAttributeTypes = (attribute, defaultLangCode, currentLangCode, domain, showLabel) => {
  ...
  if (defLangValueExists && values[defaultLangCode].type === 'file') {
    return renderAttachAttributeValue(attribute,
      defaultLangCode, currentLangCode, domain, showLabel);
  }
  if (compositeelements && compositeelements.length > 0) {
    return renderCompositeAttributeValue(attribute,
      defaultLangCode, currentLangCode, domain);
  }
  /* Check if type is link */
  if (value && (value.urlDest || value.pageDest || value.contentDest)) {
    return renderLinkAttributeValue(
      attribute, defaultLangCode, currentLangCode, showLabel,
    );
  }
  /* Check if type is either hypertext or longtext or text */
  if (defLangValueExists && typeof values[defaultLangCode] === 'string') {
    return renderTextAttributeValue(attribute,
      defaultLangCode, currentLangCode, showLabel);
  }
  /* Check if type is image */
  if (defLangValueExists && values[defaultLangCode].type === 'image') {
    return renderImageAttributeValue(attribute,
      defaultLangCode, currentLangCode, domain, showLabel);
  }
  /* Check if type is list */
  if (defLangListExists) {
    return renderListAttributeValue(attribute,
      defaultLangCode, currentLangCode, domain);
  }
  /* Check if type monolist */
  if (elements && elements.length > 0) {
    return renderMonolistAttributeValue(attribute,
      defaultLangCode, currentLangCode, domain);
  }
...
const ContentVersionDetailsValueItem = ({
  ...
  const renderedAttributeValues = renderAttributeTypes(attribute,
    defaultLangCode, currentLangCode, domain);
  return (
