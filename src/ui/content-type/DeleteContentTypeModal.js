...
const DeleteContentTypeModal = ({
  ...
  const buttons = [
    <Button bsStyle="danger" id="DeleteContentTypeModal__button-delete" onClick={() => (onConfirmDelete(info.code))}>
  ...
  return (
    <GenericModalContainer modalId={MODAL_ID} buttons={buttons} modalTitle={modalTitle} className="DeleteContentTypeModal">
      <EmptyState>
        <EmptyStateIcon name="exclamation" type="fa" className="DeleteContentTypeModal__icon" />
        <EmptyStateTitle>
          <FormattedMessage id="cms.label.delete" />&nbsp;{info.name}
        </EmptyStateTitle>
