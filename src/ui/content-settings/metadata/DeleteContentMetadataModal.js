...
const DeleteContentMetadataModal = ({
  onConfirmDelete, info,
}) => {
  const buttons = [
    <Button bsStyle="danger" id="DeleteContentMetadataModal__button-delete" onClick={() => (onConfirmDelete(info))}>
  ...
  return (
    <GenericModalContainer modalId={MODAL_ID} buttons={buttons} modalTitle={modalTitle} className="DeleteContentMetadataModal">
      <EmptyState>
        <EmptyStateIcon name="exclamation" type="fa" className="DeleteContentMetadataModal__icon" />
        <EmptyStateTitle>
          <FormattedMessage id="cms.label.delete" />&nbsp;{`"${info.key}"`}
        </EmptyStateTitle>
